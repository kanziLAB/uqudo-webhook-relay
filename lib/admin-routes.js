...
/** Manual acknowledgements — stop Uqudo's retry loop for one session. The
 *  webhook handler answers 200 for acked verification ids instead of
 *  forwarding. Ack from a failing delivery's drawer; undo re-enables
 *  forwarding for any retries still coming. */
router.get('/acks', async (req, res, next) => {
  try { res.json({ acked: await settings.ackedSessions() }); } catch (e) { next(e); }
});
router.post('/acks', async (req, res, next) => {
  try {
    const id = String((req.body || {}).verificationId || '').trim();
    if (!id) return res.status(400).json({ error: 'verificationId is required' });
    await settings.ackSession(id);
    res.json({ ok: true, acked: true, verificationId: id });
  } catch (e) { next(e); }
});
router.delete('/acks/:id', async (req, res, next) => {
  try {
    await settings.unackSession(req.params.id);
    res.json({ ok: true, acked: false, verificationId: req.params.id });
  } catch (e) { next(e); }
});
