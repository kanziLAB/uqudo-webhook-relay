...
const config = {
  ...
  uqudo: {
    ...
    // Info API for detection scores (NOT present in the raw webhook JWT — the
    // FD_* fraud rules depend on this enrichment).
    infoBaseUrl: process.env.UQUDO_INFO_BASE_URL || 'https://id.uqudo.io/api/v1/info',
    // OAuth2 client-credentials used to call the Info API.
    oauthUrl: process.env.UQUDO_OAUTH_URL || 'https://auth.uqudo.io/api/oauth/token',
    clientId: process.env.UQUDO_CLIENT_ID || '',
    clientSecret: process.env.UQUDO_CLIENT_SECRET || '',
    // Enrichment defaults ON whenever credentials are present.
    enrichWithInfoApi: bool(process.env.ENRICH_WITH_INFO_API,
      Boolean(process.env.UQUDO_CLIENT_ID && process.env.UQUDO_CLIENT_SECRET)),
    enrichTimeoutMs: num(process.env.ENRICH_TIMEOUT_MS, 12000)
  },
