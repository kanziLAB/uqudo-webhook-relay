...
// Real webhook JWTs put per-side OCR fields at scan.front / scan.back; older
// (app-processed) payloads used scan.data. Merge all three, back over front.
const scanData = (kyc) => {
  const scan = firstDoc(kyc).scan || {};
  return { ...(scan.front || {}), ...(scan.back || {}), ...(scan.data || {}) };
};
const readingData = (kyc) => ((firstDoc(kyc).reading || {}).data) || {};

// Session-level verification summary. Real webhook JWTs carry this at
// data.verifications — an ARRAY of blocks with { biometric, mrzChecksum,
// reading.passiveAuthentication, lookup } — not inside documents[].
function verificationSummary(kyc) {
  const v = kyc.verifications || (kyc.data && kyc.data.verifications);
  if (Array.isArray(v)) return v[0] || {};
  return (v && typeof v === 'object') ? v : {};
}
// Document-level face match block ({ match, matchLevel, falseAcceptRate }).
const faceBlock = (kyc) => firstDoc(kyc).face || kyc.face || (kyc.data && kyc.data.face) || {};
// Raw-JWT fields win; the enriched Info API document fills any gaps (its
// reading.data carries the NFC home-address/email/phone block).
const mergedDocData = (kyc) => {
  const info = getInfoDocuments(kyc)[0] || {};
  const infoReading = (info.reading || {}).data || {};
  return { ...infoReading, ...scanData(kyc), ...readingData(kyc) };
};

// Enriched documents fetched from the Info API (lib/enrich.js) and attached by
// server.js before mapping. The detection scores ONLY exist here — the raw
// webhook JWT does not carry them.
function getInfoDocuments(kyc) {
  const direct = kyc.infoApiDocuments;
  if (Array.isArray(direct) && direct.length) return direct;
  const nested = kyc.data && kyc.data.infoApiDocuments;
  if (Array.isArray(nested) && nested.length) return nested;
  return [];
}

/** scan.verifications, preferring the enriched Info API document. */
function bestVerifications(kyc) {
  const info = (getInfoDocuments(kyc)[0] || {});
  const infoV = (info.scan || {}).verifications || info.verifications;
  if (infoV && Object.keys(infoV).length) return infoV;
  return (firstDoc(kyc).scan || {}).verifications || {};
}

/** Detection entries are objects like { enabled, score }. Older payloads used
 *  bare `printDetection`-style keys — accept both. */
function detectionScore(v, ...keys) {
  for (const k of keys) {
    const entry = v[k];
...
/** SDK returns names like "ALEX,SAMPLE,,,TESTER," — normalise to spaces. */
...
function buildContactInfo(kyc) {
  // Real NFC-reading keys are homeAddress*-prefixed (mirrors the Flutter app's
  // _buildContactInfo). Empty email/phone here starves the MA_EM / MA_PH
  // "multiple applications with same email/phone" rules.
  const d = mergedDocData(kyc);
  return {
    mobile_phone: str(d.homeAddressMobilePhoneNo ?? d.mobilePhone),
    resident_phone: str(d.homeAddressResidentPhoneNo ?? d.residentPhone),
    email: str(d.homeAddressEmail ?? d.email),
    emirate: str(d.homeAddressEmirateDesc ?? d.emirate),
    emirate_code: str(d.homeAddressEmirateCode ?? d.emirateCode),
    city: str(d.homeAddressCityDesc ?? d.city),
    area: str(d.homeAddressAreaDesc ?? d.area),
    area_code: str(d.homeAddressAreaCode ?? d.areaCode)
  };
}
...
function buildBiometricVerification(kyc) {
  // Real webhook shape: documents[0].face = { match, matchLevel, falseAcceptRate }
  // and data.verifications[0] = { biometric:{type,matchLevel}, mrzChecksum:{valid},
  // reading:{enabled, passiveAuthentication:{documentDataSignatureValid}} }.
  // Legacy (app-processed) paths are kept as fallbacks.
  const face = faceBlock(kyc);
  const enroll = face.enrollment || {};
  const vs = verificationSummary(kyc);
  const bio = vs.biometric || {};
  const mrz = vs.mrzChecksum || {};
  const vread = vs.reading || {};
  const pauth = vread.passiveAuthentication || {};
  const doc = firstDoc(kyc);
  const rv = (doc.reading || {}).verifications || {};
  const d = mergedDocData(kyc);
  const first = (...vals) => {
  ...
  return {
    face_match: str(first(face.match, enroll.match)),
    // schema: String (not Number)
    face_match_level: str(first(face.matchLevel, enroll.matchLevel, bio.matchLevel)),
    face_false_accept_rate: str(first(face.falseAcceptRate, enroll.falseAcceptRate)),
    biometric_type: str(first(bio.type, face.type)),
    mrz_verified: str(first(d.mrzVerified, rv.mrzVerified)),
    mrz_checksum_valid: str(first(mrz.valid, rv.mrzChecksumValid)),
    passive_auth_valid: str(first(pauth.documentDataSignatureValid, rv.passiveAuthValid)),
    reading_enabled: str(first(vread.enabled, doc.reading ? true : '')),
    nfc_verified: str(first(vread.enabled, rv.nfcVerified))
  };
}

function buildFraudDetection(kyc) {
  // Real key names (from the Info API / SDK): idPrintDetection, idScreenDetection,
  // idPhotoTamperingDetection — each { enabled, score }. Legacy bare names are
  // accepted as fallback.
  const v = bestVerifications(kyc);
  return {
    // schema names are id_-prefixed for the two detection scores
    id_print_detection_score: detectionScore(v, 'idPrintDetection', 'printDetection'),
    id_screen_detection_score: detectionScore(v, 'idScreenDetection', 'screenDetection'),
    photo_tampering_score: detectionScore(v, 'idPhotoTamperingDetection', 'photoTampering'),
    is_expired: str(v.isExpired),
    is_face_matched: str(v.isFaceMatched !== undefined ? v.isFaceMatched : faceBlock(kyc).match),
...
function buildDeviceAttestation(kyc) {
  ...
  // Real webhook JWT: IP lives at info.ipInfo[0] = { ip, countryCode }.
  const ipInfo = (Array.isArray(info.ipInfo) && info.ipInfo[0]) || {};
  // Real webhook JWT has no riskCauses string — risk is a set of boolean flags.
  // Summarise the true ones so the field stays meaningful (matches the app's
  // "emulator, rooted"-style contents).
  const riskFlags = ['rooted', 'emulated', 'hooking', 'proxy', 'vpnRunning', 'debugging',
    'gpsSpoofers', 'deviceMasked', 'screenSharing', 'payloadTampered',
    'suspiciousFactoryReset', 'sdkConnectionIntercepted']
    .filter((k) => risk[k] === true);

  return {
    // Stable per-device id used by the KYC device-binding rules (OFTI_ATO_011/051).
    // Webhook JWTs call it `identifier`; app-processed payloads `deviceIdentifier`.
    deviceIdentifier: str(pick('deviceIdentifier', 'identifier', 'deviceId')),
    deviceMasked: str(pick('deviceMasked')),
    payloadTampered: str(pick('payloadTampered')),
    manufacturer: str(pick('manufacturer')),
    osVersion: str(pick('osVersion', 'version')),
    ...
    riskCauses: str(pick('riskCauses') || riskFlags.join(', ')),
    rooted: str(pick('rooted')),
    emulator: str(pick('emulator', 'emulated')),
    developerMode: str(pick('developerMode')),
    debuggable: str(pick('debuggable', 'debugging')),
    mockgps: str(pick('mockgps', 'mockGps', 'gpsSpoofers')),
    ...
    appInstallerSource: str(pick('appInstallerSource', 'applicationStore')),
    OS: str(pick('os', 'OS', 'platform')),
    ...
    IP: str(pick('ip', 'sourceIp') || ipInfo.ip),
    IPLocation_country: str(pick('ipCountry') || ipInfo.countryCode),
    ...
    IPSecurity_VPN: str(pick('vpn', 'isVpn', 'vpnRunning')),
    IPSecurity_isProxy: str(pick('isProxy', 'proxy')),
