...
function postgresDriver({ url, table, ssl }) {
  ...
  return {
    ...
    // Runtime settings (e.g. the payload-capture toggle). Persisted so the value
    // is shared across ALL serverless instances — an in-process flag would only
    // flip the one instance that handled the toggle request.
    async getSetting(key) {
      const r = await q(`SELECT value FROM relay_settings WHERE key = $1`, [key]);
      return r.rows.length ? r.rows[0].value : null;
    },
    async setSetting(key, value) {
      await q(`INSERT INTO relay_settings (key, value) VALUES ($1, $2)
...
function supabaseDriver({ url, key, table }) {
  ...
  return {
    ...
    // In-process only for the REST driver — fine self-hosted (one process),
    // per-instance on serverless. Use the postgres driver there.
    async getSetting(key) {
      return Object.prototype.hasOwnProperty.call(settings, key) ? settings[key] : null;
    },
    async setSetting(key, value) { settings[key] = String(value); }
  };
}
...
module.exports = {
  ...
  getSetting: (k) => driver.getSetting(k),
  setSetting: (k, v) => driver.setSetting(k, v),
  // exported for tests
